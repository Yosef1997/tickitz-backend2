...
exports.getMovieByMonth = async (req, res) => {
  try {
    const cond = req.query
    cond.search = cond.search || ''
    cond.page = Number(cond.page) || 1
    ...
    if (cond.search) {
      totalData = await movieModel.getCountMovieByMonth(cond)
      totalPage = Math.ceil(Number(totalData[0].totalData) / cond.limit)
    } else {
      totalData = await movieModel.getCountMovie(cond)
      totalPage = Math.ceil(Number(totalData[0].totalData) / cond.limit)
    }
    const results = await movieModel.getAllMovieByMonth(cond)
    ...
    return response(
      ...
      results,
      {
        ...
        currentPage: cond.page,
        totalPage,
        nextLink: cond.page < totalPage ? `${APP_URL}movie/month?${qs.stringify({ ...req.query, ...{ page: cond.page + 1 } })}` : null,
        prevLink: cond.page > 1 ? `${APP_URL}movie/month?${qs.stringify({ ...req.query, ...{ page: cond.page - 1 } })}` : null
      }
    )
  } catch (err) {
    console.log(err)
